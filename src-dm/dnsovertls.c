...
int dnsovertls_write (dm_query_t *p_dm_query, char *response, size_t response_len) {
  ...
  p_ssl_client=p_dm_query->p_ssl_client;
...
/* new DM query */
dm_query_t *dm_query_new(struct ssl_client *p_ssl_client)
{
    ...
    p_dm_query->p_ssl_client=p_ssl_client;       // pointer to an existing SSL struct for certs and callback
    p_dm_query->packet_write=dnsovertls_write;   //  callback to send a packet
    return p_dm_query;
}
